...
export default function LeftHeadLine({
  children,
  title,
  className,
}: {
  ...
  return (
    <div className={cn("relative flex min-h-20 w-full items-center justify-between", className)}>
      ...
      <div
        className={cn(
          "box-container relative z-10 flex w-full flex-col-reverse items-center justify-between gap-6 md:flex-row md:rtl:!flex-row-reverse",
          {
      ...
      >
        {children && children}
        <h3 className="justify-start text-start text-2xl font-semibold text-zinc-800 md:text-3xl">
          {title}
        </h3>
      </div>
    </div>
